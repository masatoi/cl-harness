;;;; src/mcp-stdio.lisp
;;;;
;;;; Phase B of the stdio-transport migration
;;;; (docs/notes/2026-05-06-stdio-transport.md). Spawns a cl-mcp
;;;; subprocess, talks JSON-RPC 2.0 over its stdin/stdout, drains its
;;;; stderr to *ERROR-OUTPUT*, and exposes the result as an
;;;; MCP-TRANSPORT instance so MCP-CLIENT can use it the same way as
;;;; the HTTP transport.
;;;;
;;;; Default command runs cl-mcp via Roswell:
;;;;   ros run -s cl-mcp -e "(cl-mcp/src/run:run :transport :stdio)" --quiet
;;;; Override per call via :COMMAND.

(defpackage #:cl-harness/src/mcp-stdio
  (:use #:cl)
...
(in-package #:cl-harness/src/mcp-stdio)
...
(defclass stdio-mcp-transport (mcp-transport)
  ((process-info :initarg :process-info :initform nil
   ...
   (next-id :initform 0 :accessor stdio-next-id)
  ...
  (:documentation
   "MCP transport that talks newline-delimited JSON-RPC over a
subprocess's stdin/stdout. PROCESS-INFO is the UIOP launch handle
(NIL when stream pair was injected directly, e.g. by tests).
PENDING maps a JSON-RPC request id to a %PENDING-ENTRY whose CV is
signalled when the matching response line arrives."))
...
matching its id. Lines without an id (server-side notifications, log
prints accidentally on stdout) and unmatched ids are dropped. Parse
errors are logged on *ERROR-OUTPUT* and otherwise tolerated, so one
...
(defun %wake-all-pending (transport reason)
  "Mark every outstanding pending entry as done with REASON in the
result slot, signal each CV, so any waiter unblocks. Used when the
...
(defmethod transport-close ((tr stdio-mcp-transport))
  (unless (stdio-closed-p tr)
    ...
    ;; Closing the streams (if subprocess was attached) lets the reader
    ;; thread see EOF and exit; if streams were injected for tests the
    ;; caller is expected to close them on its own.
    ...
    ;; Anyone still waiting (if reader exited cleanly without EOF on
    ;; the response stream — possible with injected streams) must be
    ;; woken up explicitly so they do not block forever.
...
Wraps MAKE-STDIO-MCP-TRANSPORT and feeds its result to MAKE-MCP-CLIENT
with URL nil (the HTTP-only constructor's url positional has no
meaning for stdio)."
  (make-mcp-client nil
                   :transport (make-stdio-mcp-transport
                               :command command
                               :request-timeout request-timeout)
