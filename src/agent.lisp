...
tests by editing source files via cl-mcp tools.~%~%")
    ...
    (format s "Patching guidance (backlog #38):~%")
    (format s "  - Before issuing a patch, READ the failing test and enumerate every~%")
    (format s "    symbol it references (functions, classes, accessors, slots, constructors).~%")
    (format s "  - If the test exercises multiple symbols (e.g. `make-x` + an accessor,~%")
    (format s "    defclass + its constructor, `cache-put` + `cache-get`), implement ALL~%")
    (format s "    of them in the SAME patch rather than adding them one at a time. This~%")
    (format s "    avoids `patch -> verify-fail with NEW missing symbol -> patch -> verify`~%")
    (format s "    cycles that burn turn budget without making progress.~%~%")
