...
(defun system-prompt (policy)
  "Return the Phase 2 system prompt naming the JSON action schema and
the cl-mcp tools POLICY currently permits."
...
(defun run-agent (config provider mcp-client policy logger)
  "Execute the basic Phase 2 fix loop. Returns the populated AGENT-STATE.
  ...
  (let ((state (make-instance 'agent-state)))
    ...
    (let ((initial (verify-task mcp-client config)))
      ...
      (when (verify-result-success-p initial)
        (return-from run-agent (finalize state :passed :verify initial)))
