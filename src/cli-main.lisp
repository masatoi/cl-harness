...
(defun develop-handler (cmd)
  (let* ((result
          (develop :goal (getopt cmd :goal) :project-root
           (getopt cmd :project-root) :system (getopt cmd :system) :test-system
           (getopt cmd :test-system) :test-file (getopt cmd :test-file)
           :condition (parse-condition (getopt cmd :condition)) :mcp-url
           (getopt cmd :mcp-url) :mcp-stdio (getopt cmd :mcp-stdio)
           :mcp-command (getopt cmd :mcp-command) :base-url
           (getopt cmd :base-url) :api-key (getopt cmd :api-key) :model
           (getopt cmd :model) :temperature (getopt cmd :temperature)
           :max-tokens (getopt cmd :max-tokens) :reasoning-effort
           (getopt cmd :reasoning-effort) :max-replans
           (getopt cmd :max-replans) :max-impl-review-revisions
           (getopt cmd :max-impl-review-revisions) :mode
           (parse-mode (getopt cmd :mode)) :log-path (getopt cmd :log-path)))
         (status (develop-result-status result)))
    (uiop/image:quit (develop-status-to-exit-code status))))

(defun develop-options ()
  (list
   (make-option :string :description "natural-language goal for the planner"
    :short-name #\g :long-name "goal" :required t :key :goal)
   (make-option :string :description "absolute path to the ASDF project"
    :short-name #\p :long-name "project-root" :required t :key :project-root)
   (make-option :string :description "ASDF system name" :short-name #\s
    :long-name "system" :required t :key :system)
   (make-option :string :description "ASDF test-system name" :short-name #\t
    :long-name "test-system" :required t :key :test-system)
   (make-option :string :description
   ...
   (make-option :string :long-name "log-path" :description
