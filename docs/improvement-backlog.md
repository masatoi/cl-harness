...
## 2026-05-24 JSONL aggregate analysis 由来 (#48-#52)

`docs/benchmarks/results-2026-05-24-jsonl-aggregate-analysis.md` で既存 ~60 develop-level
+ 13k step transcripts を集計分析した結果から 5 件追加。集計 metrics:
- Patch fail rate: **34% (127/374)**
- Top tool error: fs-write-file refuse (29% / 66 occurrences)
- Verify undefined-function: 67% of failures
- :max-consecutive-failed-patches (#45) は既に **3 件 fire** (works as designed)

### 48. ~~agent system prompt に fs-write-file 制限を明示~~ → 実装済 (2026-05-24)

**Status**: ✅ **実装済** — `src/agent.lisp` の system-prompt 内、fs-write-file の
schema 説明に「`Cannot overwrite existing .lisp/.asd` 診断と、source files が既に
存在する場合は lisp-edit-form insert_after で in-package form に anchor せよ」を
追加。test `system-prompt-includes-fs-write-file-workflow-guidance` で wording を pin
(`Cannot overwrite` + `lisp-edit-form` の両方を含むことを保証)。

実 effect 確認は paired bench (#46 infra で 104b 等) で別 cycle に。期待: 各 step
冒頭の fs-write-file misuse turn (現在 step あたり ~1 turn) を 0 に。



**Source**: JSONL aggregate analysis 2026-05-24
**Axis**: cl-harness 実装 (agent prompt)

**観察**: tool error 全体の **29% (66/230)** が「fs-write-file is for Lisp」混同。agent は
src/main.lisp (in-package 行のみ存在) を新規 file と誤認して fs-write-file を呼び、
tool が `Cannot overwrite existing .lisp/.asd` で reject。ほぼ毎回 lisp-edit-form の
insert-* で 1 turn 後に成功するが、各 step で **1-turn 浪費**。
...
**期待効果**: 各 step の初手 patch を確実に lisp-edit-form に向ける。N=10 paired bench data
では turn 5-7 で初 patch するパターンが平均的だが、これを turn 3-4 に短縮可。

**コスト**: small + 1 hour (prompt 修正 + agent-test で wording pin + bench で検証)。

### 49. lisp-patch-form `old_text` mismatch 時の近似 match suggestion

**Source**: JSONL aggregate analysis 2026-05-24
**Axis**: cl-mcp 実装 (tool feedback)

**観察**: tool error の **12% (27/230)** が `old_text not found in DEFUN FOO form`。agent は
file content の正確な再現を要求されるが whitespace / 改行 / 周辺 token で mismatch。
現在 error message は "matching is exact and whitespace-sensitive" の generic hint のみ。

**変更案**: cl-mcp 側 `lisp-patch-form` で mismatch 時に diff 提案を返す:
> old_text not found in DEFUN CACHE-PUT form.
> Closest substring in form: (cache-put cache key value) [Levenshtein distance: 3]
> Actual form text (first 200 chars): ...

**期待効果**: patch fail rate 34% → 25% 程度に下がれば pass rate 大幅改善。

**コスト**: medium + 1-2 days (cl-mcp 側 helper + tool error payload schema 拡張)。
cl-mcp repo の task。

### 50b. ~~planner prompt に「one new symbol per step」rule 追加 (#41/#50 強化)~~ → 実装済 (2026-05-26)

**Source**: 104-cache-simple chronic 困難 analysis 2026-05-26 (across 5 verification benches)
**Axis**: cl-harness 実装 (planner prompt)

**Status**: ✅ **実装済** — 104 の 15 trials を pass/fail で structure 比較:

| Initial plan structure | Pass / Fail | Pass rate |
|---|---|---:|
| step1 = separate `cache-put` (1 symbol) | 5/3 | **62.5%** |
| step1 = combined `cache-put-get` (2 symbols) | 2/5 | **28.6%** |

→ combined step は 1-symbol step の **半分以下の pass rate**。

planner prompt の Rules section に明示:
> Backlog #41/#50 強化 — when the goal requires defining multiple distinct
> exported symbols (e.g. a class plus several methods), prefer ONE NEW
> EXPORTED SYMBOL PER STEP. ... Avoid combined-symbol step names like
> `test-cache-put-and-get` that bundle two undefined symbols into one
> test — splitting into `test-cache-put` and `test-cache-get` keeps each
> verify failure interpretable as "this single symbol still missing"
> rather than "multiple symbols missing simultaneously" ...

test `default-planner-system-prompt-prefers-one-new-symbol-per-step` で wording を pin。

実 effect 確認は別 bench で。 期待: 104 chronic 困難の root cause 緩和、 pass rate
1/3 → 2-3/3 程度。 他 fixture (102 counter-class, 106 format-currency) でも
multi-symbol goal が対象なので transferable。

### 50. ~~planner test stub の MOP / 非標準 helper 排除 (#41 強化)~~ → 実装済 (2026-05-24)

**Status**: ✅ **実装済** — `src/planner.lisp` の `+default-planner-system-prompt+`
の "Rules" 末尾に backlog #50 として negative-list を追加:
`class-slots`, `arglist`, `function-lambda-list`, `exported-symbols`, `slot-value`,
`closer-mop` / `sb-mop` / `sb-introspect`. 代わりに `find-symbol` / accessor 経由の
assertion を提案。test `default-planner-system-prompt-bans-non-standard-test-helpers`
で 4 forbidden symbols + 標準 CL 言及を pin。

実 effect 確認は別 bench cycle で (102/100 等の trivial fixture が stuck から
PASSED に変わるか確認)。



**Source**: JSONL aggregate analysis 2026-05-24, fixture(s) 100/101/102 等
**Axis**: cl-harness 実装 (planner prompt)

**観察**: planner が test 内で MOP / 非標準 CL 関数を呼ぶ stub を生成:
...
- `EXPORTED-SYMBOLS` (過去 bench でも観察)

これは backlog #41 (planner test stub に未定義 helper を含めない) と同 issue。本 data で
**具体 symbol list が判明**したので prompt に negative example として埋め込み可能。

**変更案**: `src/planner.lisp` の planner system-prompt に追加:
...
> slot-value (use accessors instead), or any MOP function.

**期待効果**: planner generated test stub に MOP helper が含まれなくなり、解決不能 stuck
が消える。step-level :stuck の一部 (~5%) を救済。

**コスト**: small + half-day (planner prompt 修正 + planner-test regression)。

### 51. agent system prompt: file 状態 check の hint

**Source**: JSONL aggregate analysis 2026-05-24
**Axis**: cl-harness 実装 (agent prompt)

**観察**: lisp-read-file の "file not found" / "Is a directory" / fs-list-directory permission
denied が **34/230 (15%)** of tool errors。agent が exists を確認せず read を試みる。
...
**期待効果**: 各 step 前半 turn の試行錯誤 (5-10% wall-clock) 削減。

**コスト**: small + 1 hour (prompt + test)。

### 52. lisp-edit-form: form not found 時に近似 form-name suggestion

**Source**: JSONL aggregate analysis 2026-05-24
**Axis**: cl-mcp 実装 (tool feedback, #49 と同種)

**観察**: ~6 occurrences of `Form defclass simple-cache not found in src/main.lisp`。
form-name typo / form 未生成 / 別 file の confusion。
...
**コスト**: small + half-day (#49 と同 cl-mcp repo task; セット実装可)。

---

**推奨優先度** (JSONL aggregate より):

| # | Title | 影響 | コスト | ROI |
|---|---|---|---|---|
| 48 | fs-write-file workflow guidance | high (29% of errors) | small | ★★★ |
| 50 | planner stub MOP 排除 (#41 強化) | medium | small | ★★★ |
| 51 | file exists check hint | medium-low | small | ★★ |
| 49 | old_text 近似 suggest (cl-mcp 側) | medium | medium | ★★ |
| 52 | form-name 近似 suggest (cl-mcp 側) | low | small | ★★ |
...
## 2026-05-27 design review findings (DR-2026-05-27)

`/tmp/cl-harness-development-design-review.md` で develop 設計の弱点 5 件を指摘。 5 件
すべて検討 → 4 件実装、 1 件 documentation。

| # | Finding | Severity | Status |
|---|---|---|---|
| 1 | additive-only deterministic gate | High | ✅ 実装済 (d6fa5a9) |
| 2 | :test-change strict prompt | High | ✅ 実装済 (d6fa5a9) |
| 3 | review-policy default mismatch CLI vs core | Medium | ✅ docstring 明示 (b3ef5b1) |
| 4 | max-test-revisions run-wide budget | Medium | ✅ docstring 明示 (b3ef5b1) |
| 5 | test_change_request reject feedback path | Medium | ✅ 実装済 (this commit) |

### Finding 5 details (this commit)

**Problem**: `%maybe-handle-test-change-request` が rejection 時に NIL を返すと、 step は
`:test-change-request` のまま terminal に流れる。 agent に reject 理由を返して再要求 /
implementation 切替を促す path がなかった。

**Implementation**:
- `%maybe-handle-test-change-request` 戻り値を `(VALUES OUTCOME FEEDBACK)` に変更:
  - `:APPROVED nil` (従来 t と同等、 材料化 + budget 増加 + caller rerun)
  - `:REJECTED <feedback string>` (新規、 budget 増加 + feedback で agent retry)
  - `NIL nil` (review 無効 / non-test-change-request / fall through)
- 空 feedback は generic message にフォールバック (agent に signal を切らさない)
- `%enriched-issue` に `:test-change-feedback` kwarg 追加。 rendered first
  (test-change > impl-review > exploration > task) — 最も recent / specific な signal
- `%execute-step` の loop で `:rejected` を catch、 `test-change-feedback` local を
  set して次 iteration の issue 構築に乗せる + `:test-change-rejected` event を log
- budget exhaustion で従来通り step terminate (max-test-revisions counter 共有)
...
### N4.(高に昇格)compiled view に現在ソースを載せる — `:done` の単一残ブロッカー
